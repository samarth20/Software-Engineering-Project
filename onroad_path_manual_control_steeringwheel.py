...
def draw_point(world, wp, option):
    ...
    world.world.debug.draw_point(wp.transform.location + carla.Location(z=0.2), size=0.05, color=color, life_time=LIFE_TIME)


def draw_keypoint(world, location):
    world.world.debug.draw_point(location + carla.Location(z=0.2), size=0.1, color=carla.Color(128, 0, 128),
                           life_time=LIFE_TIME)
    string = "(" + str(round(location.x, 1)) + ", " + str(round(location.y, 1)) + ", " + str(round(location.z, 1)) + ")"
    world.world.debug.draw_string(location + carla.Location(z=0.5), string, True, color=carla.Color(0, 0, 128),
                            life_time=LIFE_TIME)
...
def game_loop(args):
    ...
    try:
        ...
        grp = GlobalRoutePlanner(world.map, sampling_resolution_optimal_line)
        ...
        # Logging X and Y Data for comparison and start with spawn point
        ...
        while True:
            ...
            if world.player and world.player.get_transform().location.distance(
                    route_1[-1]) > 0.8 and world.player.get_transform().location.distance(
                last_location) > sampling_resolution:
                X = np.append(X, world.player.get_transform().location.x)
                Y = np.append(Y, world.player.get_transform().location.y)
                last_location = world.player.get_transform().location
        ...
        pygame.quit()
        time.sleep(0.5)
        plt.plot(-1 * X_route[1:], Y_route[1:], 'bo',
        ...
        plt.legend()
        plt.show()
        np.savez('XY_manual_steering_wheel', X, Y)
...
def main():
    ...
    argparser.add_argument(
        ...
        # default='1280x720',
        default='1920x1080',
        help='window resolution (default: 1920x720)')
    argparser.add_argument(
        '--filter',
        metavar='PATTERN',
        default='vehicle.*',
